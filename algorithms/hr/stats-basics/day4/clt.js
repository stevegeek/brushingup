// While integration is computationally expensive here its asymtotically still O(1)
// Need to look at algos for approximating integration
...
function compute([[maxW], [boxes], [mu], [sigma]]) {
  const mu1 = boxes * mu;
  const s1 = Math.sqrt(boxes) * sigma;
  const p = cdf(0, maxW, mu1, s1);
  return [rounding(p, 4)];
}
